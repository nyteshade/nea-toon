/*
 * test_library.c - Auto-generated conformance test for toon.library
 * Generated by build_port.py from toon-format/spec test fixtures
 * Calls through toon.library via pragma dispatch.
 */
                         ...
                         const char *expected_json, int indent, int strict)
{
    char *json;
    tests_run++;
    json = ToonDecode(input, indent, strict);
    if (!json) {
        if (expected_json == NULL) {
            tests_passed++;
            return;
        }
        printf("FAIL %s: decode returned NULL\n", name);
        tests_failed++;
        return;
    }
                         ...
                         const char *expected_toon, int indent, int delim)
{
    char *toon;
    tests_run++;
    toon = ToonEncode(input_json, indent, delim);
    if (!toon) {
...
static void run_library_api_tests(void)
{
    check_int("ToonVersion returns 300", ToonVersion(), 300);
    ...
    /* JsonValue construction and query */
    {
        ...
        v = ToonNewNull();
        check_int("ToonNewNull type", ToonGetType(v), TOON_TYPE_NULL);
        ToonFreeValue(v);

        v = ToonNewBool(1);
        check_int("ToonNewBool type", ToonGetType(v), TOON_TYPE_BOOL);
        check_int("ToonGetBool value", ToonGetBool(v), 1);
        ToonFreeValue(v);

        num = 3.14;
        v = ToonNewNumber(&num);
        check_int("ToonNewNumber type", ToonGetType(v), TOON_TYPE_NUMBER);
        ToonGetNumber(v, &result);
        check_int("ToonGetNumber value", (int)(result * 100), 314);
        ToonFreeValue(v);

        v = ToonNewString("hello");
        check_int("ToonNewString type", ToonGetType(v), TOON_TYPE_STRING);
        check_str("ToonGetString value", ToonGetString(v), "hello");
        ToonFreeValue(v);
    }
    ...
    /* Array and Object construction */
    {
        ...
        arr = ToonNewArray();
        ToonArrayPush(arr, ToonNewString("a"));
        ToonArrayPush(arr, ToonNewString("b"));
        check_int("ToonArrayCount", ToonArrayCount(arr), 2);
        check_str("ToonArrayItem 0", ToonGetString(ToonArrayItem(arr, 0)), "a");
        check_str("ToonArrayItem 1", ToonGetString(ToonArrayItem(arr, 1)), "b");
        ToonFreeValue(arr);

        obj = ToonNewObject();
        ToonObjectSet(obj, "name", ToonNewString("Ada"));
        ToonObjectSet(obj, "age", ToonNewBool(0));
        check_int("ToonObjectCount", ToonObjectCount(obj), 2);
        check_str("ToonObjectKey 0", ToonObjectKey(obj, 0), "name");
        check_str("ToonGetString from obj", ToonGetString(ToonObjectItem(obj, 0)), "Ada");
        ToonFreeValue(obj);
    }

    /* Path operations via library */
    {
        char *r;

        r = ToonGet("name: Alice\nage: 30", "name", TOON_FMT_AUTO);
        check_str("ToonGet name", r, "Alice");
        ToonFreeString(r);

        r = ToonGet("name: Alice\nage: 30", "age", TOON_FMT_AUTO);
        check_str("ToonGet age", r, "30");
        ToonFreeString(r);

        r = ToonGet("items[2]: a,b", "items.1", TOON_FMT_AUTO);
        check_str("ToonGet array index", r, "b");
        ToonFreeString(r);

        r = ToonGet("name: Alice\nage: 30", ".", TOON_FMT_JSON);
        check_str("ToonGet root JSON", r, "{\"name\":\"Alice\",\"age\":30}");
        ToonFreeString(r);

        r = ToonSet("name: Alice", "name", "Bob");
        check_str("ToonSet replace", r, "name: Bob");
        ToonFreeString(r);

        r = ToonSet("name: Alice", "age", "30");
        check_str("ToonSet add key", r, "name: Alice\nage: 30");
        ToonFreeString(r);

        r = ToonDel("name: Alice\nage: 30", "age");
        check_str("ToonDel key", r, "name: Alice");
        ToonFreeString(r);
    }
    ...
    /* JSON parse/emit */
    {
        ...
        v = ToonParseJSON("{\"x\":1}", &err);
        check_int("ToonParseJSON type", ToonGetType(v), TOON_TYPE_OBJECT);
        json = ToonEmitJSON(v);
        check_str("ToonEmitJSON", json, "{\"x\":1}");
        ToonFreeString(json);
        ToonFreeValue(v);
    }
}
...
int main(void)
{
    ...
    printf("toon.library opened (spec %d.%d)\n\n",
           ToonVersion() / 100, ToonVersion() % 100);

    /* --- Spec conformance tests (same as test_runner) --- */
