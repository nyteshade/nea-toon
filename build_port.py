...
def generate_test_runner(fixtures):
    """Generate a C test runner from spec test fixtures."""
    lines = []
    lines.append('/*')
    lines.append(' * test_runner.c - Auto-generated conformance test runner')
...
def update_version_stamp(header_content, spec_version):
    """Update the $VER string and spec version in toon.h."""
    today = datetime.now()
...
def build_port(args):
    ...
    for sf in source_files:
        ...
        if sf == 'toon.h':
            content = update_version_stamp(content, spec_version)
    ...
    if with_tests:
        print("Generating test runner from spec fixtures...")
        ...
        if fixtures:
            ...
            print(f"  Generated {test_count} tests from {len(fixtures)} fixture files")
